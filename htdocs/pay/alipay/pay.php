   ...
   <td>
     <TABLE cellSpacing=0 cellPadding=0 width=740 border=0>
        ...
        <TR>
          ...
          <TD class=form-star>* </TD>
          <TD class=form-right>订单<?php echo $_POST["orderno"]?> <INPUT type="hidden" size=30 name=aliorder maxlength="200" value="<?php echo $_POST["orderno"]?>"></TD>
        </TR>
        <TR>
          <TD class=form-left>付款金额： </TD>
          <TD class=form-star>*</TD>
          <TD class=form-right><?php printf('%.2f',$_POST["price"]);?> <INPUT type="hidden" 
            onkeypress="return regInput(this,/^\d*\.?\d{0,2}$/,String.fromCharCode(event.keyCode))" 
            onpaste="return regInput(this,/^\d*\.?\d{0,2}$/,window.clipboardData.getData('Text'))" 
            ondrop="return regInput(this,/^\d*\.?\d{0,2}$/,&#9;event.dataTransfer.getData('Text'))" 
            maxLength=10 size=30 name=alimoney  onfocus="if(Number(this.value)==0){this.value='';}" value="<?php printf('%.2f',$_POST["price"])?>"/></TD>
        </TR>
        <TR>
