    ...
    $(document).ready(function(){
       var cropzoom = $('#crop_container').cropzoom({
        ...
        $('#cropbtn').click(function(){
            cropzoom.send('/?m=upload','POST',{},function(rta){
            	window.location='/?m=upload&a=view&fileurl={{$smarty.get.fileurl}}';
            });
        });
        $('#thumbbtn').click(function(){
            cropzoom.send('/?m=upload','POST',{thumb:1},function(rta){
            	//alert(rta);
            	window.location='/?m=upload&a=view&fileurl={{$smarty.get.fileurl}}';
            });
        });
        $('#backbtn').click(function(){
            window.location='/?m=upload';
        })
    })
...
.upload ul.title2 li.a1{ position:static; z-index:auto !important;  border:#DCDCDE solid 1px; border-bottom:0;height:25px !important; width:auto !important}
   ...
   <div class="upload">
    <ul class="title2">
        <li class="a1"><a href="/?m=upload&a=multiimg{{if $smarty.get.t}}&t=tinymce&input={{$smarty.get.input}}{{/if}}">批量上传</a></li>
        <li class="a2"><a href="/?m=upload&a=purview{{if $smarty.get.t}}&t=tinymce&input={{$smarty.get.input}}{{/if}}">图片列表</a></li>
		<li class="a2"><a href="/?m=upload&a=video{{if $smarty.get.t}}&t=tinymce&input={{$smarty.get.input}}{{/if}}">其他列表</a></li>
	</ul>  
    <div class="upload_a">
	...
	if(confirm("确认插入该文件吗？"))
		{
			...
			html =  html + "<div class=\"img_mc\"><img onclick=\"dotb('上传', '/?m=upload&a=crop&fileurl="+file+"&keepThis=true&TB_iframe=false&height=422&width=800');return false;\" class=\"image\" id=\"preview\" alt=\"\" src=\"{{$smarty.const.IMG_HOST}}"+file+"\" height=\"100\" title=\"点击替换主题图片\"/><input name=\"aid[]\" type=\"hidden\" value=\""+id+"\"/><p><a href=\"#\" onclick=\"$(this).parent().parent().remove();\">删除</a></p></div>";
...
.upload ul.title2 li.a1{ position:static; z-index:auto !important;  border:#DCDCDE solid 1px; border-bottom:0;height:25px !important; width:auto !important}
   ...
   <div class="upload">
    <ul class="title2">
        <li class="a2"><a href="/?m=upload&a=multiimg{{if $smarty.get.t}}&t=tinymce&input={{$smarty.get.input}}{{/if}}">批量上传</a></li>
        <li class="a2"><a href="/?m=upload&a=purview{{if $smarty.get.t}}&t=tinymce&input={{$smarty.get.input}}{{/if}}">图片列表</a></li>
        <li class="a1"><a href="/?m=upload&a=video{{if $smarty.get.t}}&t=tinymce&input={{$smarty.get.input}}{{/if}}">其他列表</a></li>
	</ul>  
...
.upload ul.title2 li.a1{ position:static; z-index:auto !important;  border:#DCDCDE solid 1px; border-bottom:0;height:25px !important; width:auto !important}
   ...
   <div class="upload">
    <ul class="title2">
        <li class="a2"><a href="/?m=upload&a=multiimg{{if $smarty.get.t}}&t=tinymce&input={{$smarty.get.input}}{{/if}}">批量上传</a></li>
        <li class="a1"><a href="/?m=upload&a=purview{{if $smarty.get.t}}&t=tinymce&input={{$smarty.get.input}}{{/if}}">图片列表</a></li>
		<li class="a2"><a href="/?m=upload&a=video{{if $smarty.get.t}}&t=tinymce&input={{$smarty.get.input}}{{/if}}">其他列表</a></li>
	</ul>  
     ...
     <form name="myform" method="POST">
       {{section name=data loop=$logs}}
     <div class="img_a"><a href="/?m=upload&a=crop&fileurl={{$logs[data].attachment}}" class="lighttxt" id="attachname{{$logs[data].id}}" title="{{$logs[data].filename}} 
...
.upload .txt22{  
    ...
    <p class="txt">
        {{include file=global/pages.tpl}} </b>
 ...
 <script type="text/javascript" src="/js/jquery/swfupload/swfobject.js"></script>
	...
	$(document).ready(function() {
		...
		$("#imgfile").uploadify({
			...
			'cancelImg': '/js/jquery/swfupload/cancel.png',
			'script': '/?m=upload',
			'buttonImg': '/js/jquery/swfupload/avatar3_btn.gif',
			'width':'298',
			'height':'57',
			'folder': '{{$path}}',
			'fileDesc': 'Image Files',
			...
			onComplete: function (evt, queueID, fileObj, response, data) {
				window.location = '/?m=upload&a=crop&fileurl='+response;
			   }
			});
