<?php /* Smarty version 2.6.7, created on 2012-09-03 14:08:45
         compiled from admin/ask.tpl */ ?>
<?php require_once(SMARTY_CORE_DIR . 'core.load_plugins.php');
smarty_core_load_plugins(array('plugins' => array(array('modifier', 'encrypt', 'admin/ask.tpl', 7, false),array('modifier', 'replace', 'admin/ask.tpl', 96, false),array('modifier', 'date_format', 'admin/ask.tpl', 98, false),)), $this); ?>
...
function Add(cateid,id)
{
	if(!id)id="";
	if(!cateid)cateid="";
	location.href="?m=<?php echo ((is_array($_tmp='ask')) ? $this->_run_mod_handler('encrypt', true, $_tmp) : encrypt($_tmp)); ?>
&a=check&cateid="+cateid+"&id="+id;
}
function Del(id){
	 if(confirm("确认删除吗？"))
		{
			$.ajax({
				type: "POST",
				url:  "?m=<?php echo ((is_array($_tmp='ask')) ? $this->_run_mod_handler('encrypt', true, $_tmp) : encrypt($_tmp)); ?>
&a=del",
				data: "id="+id,
