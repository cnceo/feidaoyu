	...
	<input type="hidden" name="id" id="id" value="{{$log.id}}">
      <div id="tab_general" style="display: block;">
        <div class="htabs" id="languages">
          ...
          {{section name=data1 loop=$ruless}}
          <tbody id="ep_row{{$smarty.section.data1.index}}">
            <tr>
              <td class="left"><select  name="ep_rules[{{$smarty.section.data1.index}}][type]">
		          ...
		          <option value="{{$data.id}}" {{if $ruless[data1].type == $k}}selected{{/if}}>{{$id}}</option>{{/foreach}}
		          </select></td>
              <td class="left"><input type="text" name="ep_rules[{{$smarty.section.data1.index}}][quantity]" value="{{$ruless[data1].quantity}}" size="4" /></td>
              <td class="left"><input type="text" name="ep_rules[{{$smarty.section.data1.index}}][price]" value="{{$ruless[data1].price|string_format:"%.2f"}}" /></td>
...
function addrules() {
    ...
    {{/foreach}}
        html += '</select></td>';		
    html += '<td class="left"><input type="text" name="ep_rules[' + ep_row + '][quantity]" value="" /></td>';
