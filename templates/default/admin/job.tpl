...
{
	if(!id)id="";
	location.href="?m={{'job'|encrypt}}&a=add&id="+id;
}
function Del(id){
	 if(confirm("确认删除吗？"))
		{
...
function Batch()
{
	...
	if(confirm("您确认"+msg+"选中的岗位吗？"))
		{
			$.ajax({
				type: "POST",
				url:  "?m={{'job'|encrypt}}&a=batch",
				data: data,
				success: function(msg){ 
						alert("操作成功");
						location.href= "?m={{'job'|encrypt}}&cateid="+$("#class_id").val();
				} 
			}); 
		}
}
...
<div class="box">    
  ...
  <div class="heading" style="overflow:hidden">
    <h1 style="background:url(/admin/images/job.png) no-repeat; overflow:hidden; width:160px">岗位列表</h1>
    <div class="buttons"><a onclick="Add();" class="button"><span>添加新岗位</span></a></div>
