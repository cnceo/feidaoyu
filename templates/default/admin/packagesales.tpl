...
{
	if(!id)id="";
	location.href="?m={{'packagesales'|encrypt}}&a=add&id="+id;
}
function Del(id){
	 if(confirm("确认删除吗？"))
		{
			$.ajax({
				type: "POST",
				url:  "?m={{'packagesales'|encrypt}}&a=del",
				data: "id="+id,
				success: function(msg){ 
				 ...
				 {
					 location.href="?m={{'packagesales'|encrypt}}";
				 }
				 else
				{
					 alert("未知错误");
        ...
        {{section name=data loop=$logs}}
          <tr>
             <td class="left">{{$logs[data].title}}</td>
              ...
              <td class="center">{{$logs[data].discount}}</td>
