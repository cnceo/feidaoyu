	 ...
	 if(confirm("确认删除吗？"))
		{
			$.ajax({
				...
				success: function(msg){
				 ...
				 {
					 location.href="?m={{'solution'|encrypt}}";
				 }
				 else
				{
					 alert("未知错误");
...
<div class="box">    
  ...
  <div class="heading" style="overflow:hidden">
    ...
    <div class="buttons"><a onclick="Add('{{'solution'|encrypt}}',null,'&cateid={{$smarty.get.cateid}}');" class="button"><span>添加方案</span></a></div>
      ...
      <table class="list">
        ...
        {{section name=data loop=$logs}}
          ...
          <td style="text-align: center;">              <input type="checkbox" name="selected[]" value="{{$logs[data].id}}" />
            ...
            <td class="left"><a href="/product/{{$logs[data].id}}.html" target="_blank">{{$logs[data].title}}</a></td>
            <td class="center">{{if $logs[data].picpath}}<img src="{{$smarty.const.IMG_HOST}}{{$logs[data].picpath}}.100x100.jpg" alt="{{$logs[data].title}}" style="padding: 1px; border: 1px solid #DDDDDD;" width="40"/>{{/if}}</td>
            <td class="right"><span>{{$logs[data].addtime}}</span></td>
            <td class="left">{{$logs[data].status_name}}</td>
            <td class="right"><a href="#" onclick="Add('{{'solution'|encrypt}}','{{$logs[data].id}}');">修改</a> |
	        	            <a href="#" onclick="Del('{{$logs[data].id}}');"  alt="Cancel"> 删   除 </a> | 
	        	             <a href="#" onclick="Add('{{'solution'|encrypt}}',null,'&cateid={{$smarty.get.cateid}}');">新加</a></td>
          </tr>
           {{sectionelse}}
          <tr>
