...
{
	if(!id)id="";
	location.href="?m={{'articlecate'|encrypt}}&a=add&id="+id;
}
function subAdd(id)
{
	if(!id)id="";
	location.href="?m={{'articlecate'|encrypt}}&a=add&pid="+id;
}
function Del(id){
	 if(confirm("确认删除吗？,删除将包括所有到子项"))
		{
			$.ajax({
				type: "POST",
				url:  "?m={{'articlecate'|encrypt}}&a=del",
				data: "id="+id,
				success: function(msg){ 
				 ...
				 {
					 location.href="?m={{'articlecate'|encrypt}}";
				 }
				 else
				{
					 alert("未知错误");
