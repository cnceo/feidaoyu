...
{
	if(!id)id="";
	location.href="?m={{'payment'|encrypt}}&a=add&id="+id;
}
function subAdd(id)
{
	if(!id)id="";
	location.href="?m={{'payment'|encrypt}}&a=add&pid="+id;
}
function Del(id){
	 if(confirm("确认删除吗？"))
		{
			$.ajax({
				...
				success: function(msg){ 
				 ...
				 {
					 location.href="?m={{'payment'|encrypt}}";
				 }
				 else
				{
					 alert("未知错误");
...
<div class="box">
  ...
  <div class="right"></div>
  <div class="heading">
    <h1 style="background-image: url('./images/payment.png');">支付方式管理</h1>
    <div class="buttons"><a onclick="Add();" class="button"><span>添加新支付方式</span></a></div>
