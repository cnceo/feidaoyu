...
function Add(cateid,id)
{
	if(!id)id="";
	if(!cateid)cateid="";
	location.href="?m={{'article'|encrypt}}&a=add&cateid="+cateid+"&id="+id;
}
function Del(id){
	 if(confirm("确认删除吗？"))
		{
			$.ajax({
				...
				success: function(msg){ 
				 ...
				 {
					 location.href="?m={{'article'|encrypt}}";
				 }
				 else
				{
					 alert("未知错误");
	...
	if(confirm("您确认"+msg+"选中的内容吗？"))
		{
			$.ajax({
				type: "POST",
				url:  "?m={{'article'|encrypt}}&a=batch",
				data: data,
				success: function(msg){ 
						alert("操作成功");
						location.href= "?m={{'article'|encrypt}}&cateid="+$("#class_id").val();
				} 
			}); 
		}
}
