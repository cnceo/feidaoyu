       ...
       $.validator.setDefaults({
			 submitHandler: function() {
			 	var data = $("#myForm").formToArray();
						$.ajax({
						type: "POST",
						url:  "?m={{'keyword'|encrypt}}&a=savePost",
						data: data,
						dataType: 'json', 
						success: function(msg){
							    ...
							    if(msg.status == "true")
								    {
										   window.location= '?m={{'keyword'|encrypt}}';    
								    }
								    else
								    {
  ...
  <div class="content">
	...
	<input type="hidden" name="id" id="id" value="{{$log.id}}">
      <div id="tab_general" style="display: block;">
			          <table class="form">
			            <tbody><tr>
			              <td><span class="required">*</span> 关键词:</td>
			              <td><input value="{{$log.keyword}}" size="20" name="keyword" id="keyword">
			                </td>
			            </tr>
			            <tr>
			              <td> 排序:</td>
			              <td><input value="{{$log.hits}}" size="10" name="hits" id="hits">
			                </td>
			            </tr>
        </tbody></table>
