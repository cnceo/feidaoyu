...
function go()
{
	location.href = "?m={{'order'|encrypt}}&a=vps&order_id="+$("#order_id").val()+"&email="+$("#email").val()+"&host="+$("#host").val()+"&status="+$("#status").val();
}
  ...
  <div class="content">
      ...
      <table class="list">
        ...
        <tr class="filter">
            ...
            <td class="left"><input type="text" value="{{$smarty.get.host}}" id="host"></td>
            <td></td>
            <td class="center"><select id="status">
                                 <option value="">全部</option>
                                {{html_options options=$vpsstatus selected=$smarty.get.status}}
                              </select></td>
        ...
        {{foreach from=$logs item=data}}
          ...
          <td style="text-align: center;">              <input type="checkbox" name="selected[]" value="{{$data.id}}" />
            ...
            <td class="left">{{$data.cprodname}}</td>
            <td class="left">{{$data.year}}年</td>
            <td class="center">{{if $data.status==1}}已付款{{elseif $data.status==2}}已经开通{{else}}未付款{{/if}}</td>
            <td class="center"><a href="?m={{'order'|encrypt}}&a=vps_detail&id={{$data.id}}" target="_blank" >查看</a></td>
