 ...
 */
   ...
   class Files
   {
   	   ...
   	   public function __construct()
   	    ...
   	    public function upfile()
   	    {
			...
			if (in_array($this->ext,array("jpg","gif","png","bmp")))
			{
				...
				$this->SaveToDb($file,$width,$filesize,1,$this->mUid,$this->mType,1);

			}else
			{
				$this->SaveToDb($file,0,$filesize,0,$this->mUid,$this->mType,0);
    ...
    public function SaveToDb($file,$width,$filesize,$isimage,$uid,$type,$thumb)
		{
			$sql = "INSERT INTO ".get_table("attachments")." (width,dateline,filename,filetype,filesize,attachment,isimage,uid,utype,thumb)
			VALUES ('$width', '".time()."', '".basename($file)."', 'pplication/octet-stream', '$filesize', '$file', '$isimage', '$uid', '$type', '$thumb')";
