<?php
	class Activitys
	{
		...
		{
			$sql = "SELECT * FROM ".get_table("activitys");
		...
		public function getNavProductList($cateid,$limit=3)
		{
			...
			$sql = "SELECT * FROM ".get_table("activitys")." WHERE class_id IN ($cateid) AND status=1 LIMIT $limit";
		...
		{
			$sql = "SELECT * FROM ".get_table("activitys")." WHERE sold_quantity >0 ORDER BY sold_quantity DESC LIMIT $limit";
			$rs = $this->mDb->getall($sql);
			return $rs;
		}
		...
		{
			$sql = "SELECT * FROM ".get_table("activitys")." WHERE hits >0 ORDER BY hits DESC LIMIT $limit";
			$rs = $this->mDb->getall($sql);
			return $rs;
		}
		...
		{
			$sql = "SELECT * FROM ".get_table("activitys")." WHERE  sbstar>0 AND status=1 ORDER BY sbstar DESC LIMIT $limit";
			$rs = $this->mDb->getall($sql);
			return $rs;
		}
		...
		public function savePost()
		{
			...
			if(!$_POST["id"])
			{
				$sql = "INSERT INTO ".get_table("activitys")." SET ".implode(",",$sqlv);

			}
			else
			{
				$sql = "UPDATE ".get_table("activitys")." SET ".implode(",",$sqlv)." WHERE id=".$_POST["id"];
			}
			...
			$this->updateAttachments($aid,$pid,"activity");
			return $rs;
 		}
 		...
 		public function Batch()
	  	{
	  		...
	  		while ($i<count($_POST["selected"]))
	  		{
	  			if($_POST["more"]=="0")
	  			{
	  				$sql = "DELETE FROM ".get_table("activitys")." WHERE id=".$_POST["selected"][$i];
	  			}
	  			else if($_POST["more"]=="3")
	  			{
	  				$sql = "UPDATE ".get_table("activitys")." SET status='1' WHERE id=".$_POST["selected"][$i];
	  			}
	  			else if($_POST["more"]=="4")
	  			{
	  				$sql = "UPDATE ".get_table("activitys")." SET status='0' WHERE id=".$_POST["selected"][$i];
	  			}
	  			else
	  			{
	  				$sql = "UPDATE ".get_table("activitys")." SET class_id='".$_POST["class_id"]."' WHERE id=".$_POST["selected"][$i];
	  			}
	  			$rs = $this->mDb->execute($sql);
	  			$i++;
	  		}
		...
		public function saveUpdate()
		{
			if(is_array($_POST["seque"]))
			{
				foreach($_POST["seque"] as $key => $val)
		 		{
	 				if($val=="")$val=0;
	 				$sql = "UPDATE ".get_table("activitys")." SET sequence='".$val."' WHERE id = $key";
		...
		{
			$sql = "SELECT * FROM ".get_table("activitys").$where;
			$rs = $this->mDb->getall($sql);
			if(!empty($type))
			{
				$cate = array();
		...
		{
			$sql = "SELECT * FROM  ".get_table("attachments")." WHERE pid=$pid AND utype='activity'";
